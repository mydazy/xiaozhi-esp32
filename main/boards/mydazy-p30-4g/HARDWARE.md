...
- **镜像/旋转**：逆时针 90° 旋转（SWAP_XY + MIRROR_X）
- **TE 信号**：硬件连接到 GPIO40，软件**未启用 VSYNC 同步**

### 3.3 触摸系统
- **驱动 IC**：AXS5106L（本项目自研驱动）
- **I2C 地址**：`0x63`（7位，定义于 `axs5106l_touch.cc:20`）
- **总线**：I2C1（与音频编解码共用）
- **I2C 速率**：400kHz（官方推荐）
- **中断**：硬件 INT 信号（GPIO5）确认触摸事件，避免 4G RF 干扰
- **手势支持**：单击/双击/长按/长按释放/上滑/下滑/左滑/右滑
- **固件升级**：`axs5106l_upgrade.cc` + 固件 blob 1.6MB（Boot 时自动校验升级）
- **软件复位命令**：`{0xB3, 0x55, 0xAA, 0x34, 0x01}` 写入触摸屏
