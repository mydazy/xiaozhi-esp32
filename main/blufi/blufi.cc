...
void Blufi::ReleaseStaticMem() {
  ...
  if (ret == ESP_OK) {
    ...
    ESP_LOGI(TAG, "BT 静态 RAM 已释放 %u B (internal free %u -> %u)",
             (unsigned)(free_after - free_before), (unsigned)free_before, (unsigned)free_after);
...
void Blufi::BlufiCallback(esp_blufi_cb_event_t event,
  ...
  case ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP: {
    ESP_LOGD(TAG, "[7/8] 收到WiFi连接请求: %s", self.ssid_.c_str());
    ...
    xTaskCreatePinnedToCore([](void *arg) {
      ...
      if (self.credential_validator_(self.ssid_, self.password_, error)) {
        ...
        esp_blufi_send_wifi_conn_report(WIFI_MODE_STA, ESP_BLUFI_STA_CONN_SUCCESS,
                                        0, &info);
        ESP_LOGD(TAG, "[7/8] 配网成功: %s", self.ssid_.c_str());
      } else {
  ...
  case ESP_BLUFI_EVENT_RECV_STA_SSID:
    ...
    ESP_LOGD(TAG, "[4/8] 收到SSID: %s", self.ssid_.c_str());
    break;
  ...
  case ESP_BLUFI_EVENT_RECV_CUSTOM_DATA:
    ...
    ESP_LOGD(TAG, "[6/8] 收到绑定码: %s", self.binding_code_.c_str());
