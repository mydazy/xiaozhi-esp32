...
// 闹钟"唤醒词"拼接 · 短促像用户呼叫设备 · AI 把这当对话起点
// 用 WakeWordInvoke 触发 · AI 走正常对话链路（含 listening → 用户响应）
static std::string BuildWakeWord(const std::string& message, int elapsed_s) {
    ...
    if (message.empty()) {
        ...
        snprintf(buf, sizeof(buf), "闹钟已响 %d 秒，主人还没回应，再叫一次", elapsed_s);
    } else {
        if (elapsed_s == 0) {
            snprintf(buf, sizeof(buf), "闹钟响了，提醒主人：%s", message.c_str());
        } else {
            snprintf(buf, sizeof(buf),
                     "闹钟响 %d 秒了，主人没回应，再提醒：%s",
                     elapsed_s, message.c_str());
        }
    }
    return buf;
}
