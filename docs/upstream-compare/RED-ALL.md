...
**死码已清 13 项**（commit `93d80549f` 清 9 项 + 本批清 4 项）：
- 本批 4 项：🔴10 `RequestSwitch`（NFC 切换；硬件保留、程序本版删、下版重加）· 🔴11 OnSleep 的 esp_restart 死分支 · 🔴12 download 空命令 · 🔴13 remote_cmd.h 文档同步
- ⚠️ ota.cc 红线已核实：连带删的 `PostToOta` 仅服务 RequestSwitch；`ReportStatus`（自有 POST）/`Download`/`CheckVersion`/`Upgrade`/`Activate` 核心**未触碰**

**保留不恢复（附原因）**：
- 🔴14 PLC 补偿 / 🔴15 音量 ×0.95 → **待真机弱网/PA 实测**，不盲目回退
- 🔴16 RF 参数 → **冻结**（停止调参，不动代码）
- 🔴17 custom 语义 → **纠正：实为业务必需扩展**（远程命令唯一入口，恢复会废掉 19 个命令），非过度优化，保留

**待修（独立排期，非过度优化）**：3 个量产必修 P1（见 `P1-FIX-PLAN.md`）+ 6 个 🔒 安全项（含 2 个 P1 级）
