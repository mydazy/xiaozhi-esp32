...
任务按 CLAUDE.md 规范分配：网络/编解码/主循环上 Core 0，音频实时/AEC/UI 上 Core 1。优先级从 P10（最高实时）到 P1（后台）。

✅ **2026-04-29 P1 整改完成 · 漂移清零**：`main/` 下全部 32 处 `xTaskCreate*` 调用确认 Pin 到指定核（grep `xTaskCreate\b` = 0 命中）。本轮修 14 处历史漂移任务 + 2 处 PSRAM/Core0 红线。

✅ **2026-04-29 双核重平衡 · 解决 Task WDT reboot**：原方案 `audio_input` 在 Core 0 + `opus_codec` 在 Core 0 → Core 0 占用 ~100% 触发任务看门狗。重平衡后 Core 0 ~50% / Core 1 ~63%，实时音频/AEC 抢占 LVGL（用户无感 30→25fps）。详见 § 四.0。

### 四、0 · 双核分配最终方案（2026-04-29 重平衡 · 权威源）

```
Core 0（网络/协议栈/编解码/主循环 · 占 ~50%）        Core 1（实时音频/AEC/UI/唤醒 · 占 ~63%）
┌──────────────────────────────────────────────┐  ┌──────────────────────────────────────┐
│ P10  main_app          (8KB INT · ✅ Pin)    │  │ P10  audio_input  (✅ Pin Core 1)    │
│ P10  audio_output      (✅ Pin · 实时 DAC)   │  │ P7   audio_communication (AFE+AEC)   │
│ P7   opus_codec        (24KB · 编解码密集)   │  │ P7   audio_detection (WakeNet)       │
│ P5   ml307_net         (✅ Pin)              │  │ P7   mp3_decode / mp3_play / mp3_out │
│ P4   status_assets/OTA (✅ Pin)              │  │ P5   LVGL                            │
│ P3   wifi/modem/lwip                         │  │ P5   blufi_wifi (✅ Pin)             │
│ P2   activation        (✅ Pin)              │  │ P5   config_done (✅ Pin)            │
│ P2   LedEvent          (✅ Pin)              │  │ P3   config_switch ×2 (✅ Pin)       │
│ P2   alarm_manager     (✅ Pin)              │  │ P2   encode_wake_word ×2 (✅ Pin)    │
│ P1   flow_load                               │  │ P2   headset_detect                  │
│                                              │  │ P1   stt_post (INT/Core1 · P0 修)    │
└──────────────────────────────────────────────┘  └──────────────────────────────────────┘
```

**关键设计原则**（不要再翻案）：
- **`audio_input` 必须 Core 1** — AEC VOIP_HIGH_PERF 30~50ms/帧，Core 0 装不下（实测 Task WDT 被触发的根因）
- **`opus_codec` 必须 Core 0** — 同核会让 Core 1 总占用爆 88%；跨核通过 mutex queue 0 开销
- **`audio_output` 跟 `opus_codec` 同 Core 0** — DAC 写与编解码同核，避免跨核 cache miss
...
|-------|------|----------|--------|------|---------|------|
| `main_app` | `main.cc:14` | 8192 | **P10** | **Core 0 ✅** | 永久 | `Application::Run()` 主事件循环 |
| `audio_input` | `audio_service.cc:137/151` | 6144/4096 | **P10** ⬆ | **Core 1 ✅** ⬆ | Start→Stop | I2S 采集 + AFE Feed（2026-04-29 双核重平衡：Core 0→Core 1 · P8→P10 · AEC 计算密集需独占核） |
...
| `mp3_play / mp3_out` | `audio/music_player.cc` | INT | P10 | Core 1 | 触发→完成 | MP3 PCM 播放（与 audio_output 互斥 · 通过 ResetDecoder 切换） |
| `mp3_dl` | `audio/music_player.cc` | PSRAM | P1 | Core 1 | 触发→完成 | MP3 HTTP 下载 |
| `activation` | `application.cc:304` | 8192 | P2 | **Core 0 ✅** | 网络上线→完成 | OTA 激活 + 协议初始化（2026-04-29 P1：显式 Pin Core 0） |
| `status_assets` | `ota.cc` | 4096 | P4 | **Core 0 ✅** | 触发→完成 | OTA 状态上报 + 资源拉取 |
| `stt_post` | `remote_cmd.cc` | 4096 INT | P1 | **Core 1 ✅** | 触发→完成 | STT 文本回调 HTTP POST（2026-04-28 P0：PSRAM/Core0 → INT/Core 1） |
| `flow_load` | `flow_engine.cc` | 6144 | P1 | Core 0 | 触发→完成 | FlowEngine HTTP 脚本加载（一次性自删） |
| `ml307_net` | `boards/common/ml307_board.cc:148` | 4096 | P5 | **Core 0 ✅** | 网络启动→停止 | 4G modem AT/PPP 网络任务 |
| `LedEvent` | `led/gpio_led.cc:86` | 2048 | P2 | **Core 0 ✅** | 永久 | LED 渐变事件循环 |
| `alarm_manager` | `alarm_manager.cc` | 4096 | P2 | Core 0 | 永久 | 闹钟调度（依赖 esp_timer · 与 main_app 同核） |
| `blufi_wifi` | `blufi/blufi.cc` | 4096 | P5 | **Core 1 ✅** | BluFi 配网中 | BluFi WiFi 协议任务（BLE+WiFi 同核避免协议栈跨核） |
| `config_done` | `boards/common/wifi_board.cc:444` | 8192 | P5 | **Core 1 ✅** | 配网成功→重启 | 配网完成清理 + Reboot |
| `config_switch` (4G/WiFi) | `boards/mydazy-p30-{4g,wifi}/...:570/569` | 4096 | P3 | **Core 1 ✅** | 双击触发→完成 | BLUFI ↔ AP 配网模式切换 |
| `wifi_ap` ×4 | `wifi/wifi_ap.cc` | 配置项 | - | Core 1 | AP 模式中 | AP 配网协议栈 |
| `dns_server` | `wifi/dns_server.cc` | 2048 | P5 | Core 1 | AP 模式中 | 配网 DNS |
| `headset_detect` | `boards/common/typec_headset.cc` | 3072 | P3 | Core 0 | Init→Destroy | Type-C 耳机检测 |
| `lvgl_task` | (esp_idf 内置) | 配置项 | P5 | Core 1 | 永久 | LVGL 图形引擎 |
| `wifi_task` | (esp_idf 内置) | 配置项 | P23 | Core 0 | 网络启动→停止 | WiFi 驱动 |
| `tcpip_thread` | (esp_idf 内置) | 配置项 | P18 | Core 0 | 永久 | LWIP TCP/IP |
| `clock_timer` | esp_timer 服务 | 3584 | P22 | Core 0 | 永久 | 周期 tick |

⬆ = 2026-04-29 双核重平衡变更点

**Core 落位统计**（main/ 32 处 Pin 调用 · grep 验收 0 漂移）：

- **Core 0**（网络/编解码/主循环 = 8 task）：main_app、audio_output ×2 分支、opus_codec、ml307_net、status_assets、activation、LedEvent、alarm_manager、headset_detect、flow_load
- **Core 1**（实时音频/AFE/UI = 11 task）：audio_input ×2 分支、audio_communication（AFE）、audio_detection（WakeNet）、encode_wake_word ×2、mp3_decode、mp3_play、mp3_out、mp3_dl、stt_post、blufi_wifi、config_done、config_switch ×2、LVGL、dns_server、wifi_ap ×4
- **2026-04-29 重平衡变更**：`audio_input` Core 0→Core 1 · `audio_output` P4→P10 · `audio_input` P8→P10 · `audio_communication` P3→P7（解决 Task WDT reboot · 详见 § 四.0）
...
### 四、A · sdkconfig 优化总览（2026-04-29 完成态 · 权威源 `sdkconfig.defaults`）

#### A · 长期稳定 ✅ · 5 类 Watchdog（P0-3 完成）

| 配置项 | 值 | 作用 |
|---|---|---|
| `ESP_TASK_WDT_EN` / `INIT` | y / y | 任务看门狗启用 + 启动期初始化 |
| `ESP_TASK_WDT_TIMEOUT_S` | **10** | 任务卡死 10s 触发 panic + 重启 |
| `ESP_TASK_WDT_CHECK_IDLE_TASK_CPU0` / `CPU1` | y / y | 双核 idle 任务监控 |
| `ESP_INT_WDT` / `TIMEOUT_MS` | y / **300** | 中断/调度卡死 300ms 触发 panic |
| `ESP_INT_WDT_CHECK_CPU1` | y | 监控 Core 1 中断 |
| `FREERTOS_CHECK_STACKOVERFLOW_CANARY` | y | 栈越界 canary 检测 |
| `ESP_BROWNOUT_DET` / `LVL_SEL_7` / `USE_INTR` | y / y(2.6V) / y | 电压跌落检测 · 中断方式 |
| `FREERTOS_USE_TRACE_FACILITY` | y | uxTaskGetSystemState · high-water mark |
| `FREERTOS_GENERATE_RUN_TIME_STATS` | y | CPU 占用率统计 |

#### B · INT RAM 优化 ✅ · P0a 主动改（释放 ~21-30 KB INT）

| 配置项 | 原值 | 新值 | INT 释放 | 副作用 |
|---|---|---|---|---|
| `BT_NIMBLE_MEM_ALLOC_MODE_INTERNAL` | y | **n** | — | 关 INT 模式 |
| `BT_NIMBLE_MEM_ALLOC_MODE_EXTERNAL` | n | **y** | **+15-20 KB** | NimBLE 挪 PSRAM · BluFi 启动慢 ~50ms |
| `LWIP_TCP_WND_DEFAULT` | 5760 | **4096** | **+3-5 KB** | 单 TCP 接收吞吐略降 |
| `LWIP_TCP_SND_BUF_DEFAULT` | 5760 | **4096** | **+3-5 KB** | 单 TCP 发送吞吐略降 |
| **合计** | — | — | **+21-30 KB INT** | HTTP Binary 已对冲带宽损失 |

#### C · 已启用的内存优化（继承 · 无需再动）

`COMPILER_OPTIMIZATION_SIZE=y` · `NEWLIB_NANO_FORMAT=y` · `MBEDTLS_DYNAMIC_BUFFER=y` · `MBEDTLS_SSL_KEEP_PEER_CERTIFICATE=n` · `MBEDTLS_SSL_RENEGOTIATION=n` · `ESP_WIFI_STATIC_RX_BUFFER_NUM=6` · `ESP_WIFI_DYNAMIC_RX_BUFFER_NUM=8` · `ESP_WIFI_DYNAMIC_RX_MGMT_BUFFER=y` · `ESP_WIFI_IRAM_OPT=n` · `ESP_WIFI_RX_IRAM_OPT=n` · `ESP_WIFI_ENTERPRISE_SUPPORT=n` · `ESP_MAIN_TASK_STACK_SIZE=8192` · `UART_ISR_IN_IRAM=y` · `SPIRAM_MALLOC_RESERVE_INTERNAL=98304`

#### D · 业务/启动配置

`BOOTLOADER_COMPILER_OPTIMIZATION_PERF=y` · `BOOTLOADER_LOG_LEVEL_NONE=y` · `BOOTLOADER_SKIP_VALIDATE_ALWAYS=y` · `BOOTLOADER_APP_ROLLBACK_ENABLE=y` · `PARTITION_TABLE_CUSTOM=partitions/v2/16m.csv` · `HTTPD_MAX_REQ_HDR_LEN=2048` · `HTTPD_MAX_URI_LEN=2048` · `EN_SC7A20H_WAKE=y` · `LV_USE_GIF=y` · `LV_USE_LODEPNG=y` · LVGL 裁剪关 14 个 widget · `ESP_CONSOLE_UART_BAUDRATE=2000000`

#### 下一轮 P1 候选（量产稳定后再做）

1. `SPIRAM_FETCH_INSTRUCTIONS=y` 把部分指令搬 PSRAM（释放 ~30 KB Flash · cache miss 风险需评估）
2. `LWIP_MAX_SOCKETS` 10 → 8（释放 ~2 KB · 评估业务并发数）

### 四、B · 2026-04-29~30 优化清单（量产前最后一轮）

#### B.1 · 双核重平衡（解决 Task WDT reboot · 2026-04-29）

**根因**：原方案 Core 0 同时跑 `main_app + audio_input + opus_codec + ml307_net + audio_output` → 实测 Core 0 ~100% 占用，audio_input 中 AFE Feed（VOIP_HIGH_PERF AEC 30~50ms/帧）触发任务看门狗 panic。

**重平衡 4 处改动**（grep 实地验证）：

| 任务 | 旧 Core/Prio | 新 Core/Prio | 文件 | 理由 |
|---|---|---|---|---|
| `audio_input` | Core 0 / P8 | **Core 1 / P10** | `audio_service.cc:137,151` | AEC 计算密集 → Core 1 独占 |
| `audio_output` | Core 0 / P4 | **Core 0 / P10** | `audio_service.cc:144,158` | 实时 DAC 抢占级 · 与 codec 同核 |
| `audio_communication` | Core 1 / P3 | **Core 1 / P7** | `afe_audio_processor.cc:74` | AFE ringbuffer 防饥饿 |
| `opus_codec` | Core 0 / P7 | **Core 0 / P7** | `audio_service.cc:166` | 不动 · 必须留 Core 0 否则 Core 1 占用爆 88% |

**实测占用**：Core 0 ~50% / Core 1 ~63%（LVGL 帧率 30→25fps 用户无感）。

#### B.2 · NimBLE role 裁剪 + 弱网防御（2026-04-29 P0）

| 改动 | 配置/文件 | 收益 |
|---|---|---|
| 关 BLE CENTRAL/OBSERVER/GATT_CLIENT | `sdkconfig.defaults` `BT_NIMBLE_ROLE_CENTRAL=n` 等 | 释放 ~5 KB INT |
| W5 · MP3 ring 水位检测（弱网防爆音） | `audio/music_player.cc` | 弱网下 MP3 不再卡顿+爆音 |
| W4 · Listening 期 30s 超时 timer **删除** | `application.cc` `response_timeout_timer_` 全删 | 30s MP3 不再被误杀（用户反馈"播一半断"根因） |

#### B.3 · ML307 4G 双补丁（2026-04-29 / 30）

| 补丁 | 文件 | 内容 | 收益 |
|---|---|---|---|
| Patch A · 锁 LTE-only | `ml307_at_modem.cc` `ConfigureRadioProfile` | 启动时 AT+CFUN 锁 LTE | 拒 2G/3G 注册 · 弱网更稳 |
| Patch B · HTTP Binary 模式 | `at_uart.{h,cc}` + `ml307_http.{h,cc}` | binary 替代 base64 | 上行带宽 +30% · 抵消 LWIP TCP_WND 4096 损失 |

#### B.4 · 4G 深度睡眠流程修复（2026-04-30）

**根因**：USB 连接时设备睡眠会立即被 `ESP_RST_USB` 唤醒；MQTT 异步保活包路径上 `PowerSaveTimer::Exit()` 与睡眠流程竞争。

**修复**（3 板同步 · `mydazy_p30_4g_board.cc` / `mydazy_p30_wifi_board.cc` / `mydazy_p31_board.cc`）：

```
SetEnabled(false) → 关背光 → ResetProtocol（主动断 MQTT）→ vTaskDelay(500)
   → ShutdownTouchAndAudio → ConfigureWakeup → WiFi.Stop → ResetGpios → esp_deep_sleep_start
```

#### B.5 · UI 状态栏图标节流（2026-04-30）

`ui_display.cc::UpdateGlobalStatusIcons` 加节流：电池查询 10s/次（间隔从 1s 拉长 10×），网络信号 5s/次。收益：减少 90% 的 ML307 `AT+CSQ` 调用频次，避免 modem AT 队列拥塞。

#### B.6 · 说话结束提示音（2026-04-30 · 来自 189 移植）

| 改动 | 位置 | 行为 |
|---|---|---|
| 收到服务器 STT 文本时播 OGG_POPUP | `application.cc::OnIncomingJson stt` | 用户立即知道"服务器收到我的语音" |
| 跳过条件 | `skip_next_stt_popup_` atomic | ① PTT 模式 ② 唤醒词首条 STT ③ 用户关闭 |
...
```cpp
// 2026-04-29 P0+ · PCM 缓冲从 2 帧（120ms）放大到 4 帧（240ms）
// 抵御 LVGL 全屏刷+NVS commit+WakeNet 推理+OTA chunk 写累积突发
// RAM 增量：encode +3.8K PSRAM · playback +5.7K PSRAM · INT 几乎不变
// 不放更大（如 6/360ms）：超 250ms 上行用户感知"延迟"
#define MAX_ENCODE_TASKS_IN_QUEUE  4
#define MAX_PLAYBACK_TASKS_IN_QUEUE 4
#define MAX_DECODE_PACKETS_IN_QUEUE (2400 / OPUS_FRAME_DURATION_MS)  // 40 包 · 2400ms
#define MAX_SEND_PACKETS_IN_QUEUE   (2400 / OPUS_FRAME_DURATION_MS)  // 40 包 · 2400ms
...
[L2] PushTaskToEncodeQueue    注入点 E（等价 D · 延后 1 frame · 实现更干净）
   ↓
[L4] opus_codec (Core 0 P7)   ❌ Opus 压缩域 · 改不了
   ↓ protocol.SendAudio → MQTT/WS → 服务器
```
