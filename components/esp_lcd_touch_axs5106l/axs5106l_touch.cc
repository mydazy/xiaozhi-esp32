...
// 抗干扰参数（平衡 4G 抗干扰 vs 触摸流畅度）
#define MIN_TOUCH_INTERVAL   30000    // 最小触摸间隔(us) - 30ms（快速点击响应）
#define STABLE_TOUCH_COUNT   1        // 首帧即确认（INT去抖+按下窗口+速度滤波已覆盖RF防护）
#define STABLE_TOUCH_DIST    30       // 稳定触摸的位置容差(像素)
#define MAX_SPEED_THRESHOLD  2000     // 速度过滤阈值(px/s)（正常快速滑动~1400，RF跳变>3000）
#define INT_DEBOUNCE_US      3000     // INT 引脚去抖时间(us)（非阻塞，帧间确认）
#define PRESS_CONFIRM_US     20000    // 按下确认窗口(us) - 20ms

// 释放去抖参数
#define RELEASE_DEBOUNCE_COUNT  2     // 连续2帧释放确认

// 连续异常帧抑制参数
#define INVALID_FRAME_THRESHOLD  5    // 连续5帧异常才暂停
#define INVALID_SUPPRESS_TIME_US 300000  // 暂停300ms

// I2C 参数
#define I2C_TIMEOUT_MS    100         // I2C超时（给 4G RF 干扰恢复时间）
...
Axs5106lTouch::Axs5106lTouch(i2c_master_bus_handle_t i2c_bus,
    ...
    // 初始化抗干扰状态
    anti_interference_ = {0, false, 0, 0, 0, 0, 0, 0};
}
...
void Axs5106lTouch::LvglReadCallback(lv_indev_t* indev, lv_indev_data_t* data) {
    ...
    if (self == nullptr || self->sleeping_) {
    ...
    if (self->ParseTouchData(x, y, pressed)) {
        if (!self->anti_interference_.is_touch_active) {
            // 新触摸开始：时间窗口确认（非阻塞，跨帧）
            uint64_t now = esp_timer_get_time();
            if (self->anti_interference_.press_first_time == 0) {
                self->anti_interference_.press_first_time = now;
            }
            if (now - self->anti_interference_.press_first_time < PRESS_CONFIRM_US) {
                data->state = LV_INDEV_STATE_RELEASED;
                return;
            }
            self->anti_interference_.press_first_time = 0;
        }

        // 有效触摸数据
        self->anti_interference_.release_count = 0;
        self->anti_interference_.is_touch_active = true;
        self->anti_interference_.last_valid_x = x;
        self->anti_interference_.last_valid_y = y;
        ...
        // 释放去抖
        if (self->anti_interference_.is_touch_active) {
            self->anti_interference_.release_count++;

            if (self->anti_interference_.release_count >= RELEASE_DEBOUNCE_COUNT) {
                self->anti_interference_.is_touch_active = false;
                self->anti_interference_.release_count = 0;
                data->point.x = self->anti_interference_.last_valid_x;
                data->point.y = self->anti_interference_.last_valid_y;
                data->state = LV_INDEV_STATE_RELEASED;
...
bool Axs5106lTouch::ParseTouchData(uint16_t& x, uint16_t& y, bool& pressed) {
    ...
    // 防护层0: 连续异常帧抑制期，跳过所有处理
    if (now < anti_interference_.suppress_until) {
        pressed = false;
        return false;
    }

    // 防护层1: INT 引脚非阻塞去抖（跨帧确认）
    // AXS5106L INT: 触摸时持续低电平，无触摸时高电平
    int int_level = gpio_get_level(int_gpio_);

    if (int_level == 0) {
        // INT 低电平 → 可能有触摸
        if (!anti_interference_.is_touch_active) {
            if (anti_interference_.int_low_since == 0) {
                anti_interference_.int_low_since = now;
            }
            if (now - anti_interference_.int_low_since < INT_DEBOUNCE_US) {
                pressed = false;
                return false;  // 去抖窗口内，不读 I2C
            }
        }
    } else {
        // INT 高电平 → 无触摸
        anti_interference_.int_low_since = 0;
        if (anti_interference_.is_touch_active) {
            anti_interference_.invalid_frame_count = 0;
        }
        pressed = false;
        if (filter_.count > 0) {
            filter_.count = 0;
    ...
    if (!ReadRegister(AXS5106L_REG_DATA, buf, 8)) {
        anti_interference_.invalid_frame_count++;
        if (anti_interference_.invalid_frame_count >= INVALID_FRAME_THRESHOLD) {
            anti_interference_.suppress_until = now + INVALID_SUPPRESS_TIME_US;
            anti_interference_.invalid_frame_count = 0;
            ESP_LOGW(TAG, "连续%d帧I2C失败，暂停触摸%dms", INVALID_FRAME_THRESHOLD, INVALID_SUPPRESS_TIME_US / 1000);
        }
        pressed = false;
        return false;
    }
    ...
    if (points == 0 || points > 1) {
        ...
        if (anti_interference_.invalid_frame_count >= INVALID_FRAME_THRESHOLD) {
            anti_interference_.suppress_until = now + INVALID_SUPPRESS_TIME_US;
            anti_interference_.invalid_frame_count = 0;
            ESP_LOGW(TAG, "连续%d帧数据异常，暂停触摸%dms", INVALID_FRAME_THRESHOLD, INVALID_SUPPRESS_TIME_US / 1000);
        }
        pressed = false;
        if (filter_.count > 0) {
            filter_.count = 0;
    ...
    if (raw_x > TOUCH_MAX_X + 50 || raw_y > TOUCH_MAX_Y + 50) {
        return false;
    }

    // 坐标缩放：触摸分辨率(240×284) → 屏幕分辨率(284×240)
    uint16_t screen_x = (uint16_t)((uint32_t)raw_x * (width_ - 1) / (TOUCH_MAX_X - 1));
    uint16_t screen_y = (uint16_t)((uint32_t)raw_y * (height_ - 1) / (TOUCH_MAX_Y - 1));
    ...
    filter_.last_x = sum_x / filter_.count;
    filter_.last_y = sum_y / filter_.count;
...
void Axs5106lTouch::RecognizeGesture(int16_t x, int16_t y, bool pressed) {
    ...
    if (pressed && !gesture_.is_pressed) {
        ...
        // 长按已触发 → 松开时发送 LongPressRelease
        if (gesture_.long_press_triggered) {
            gesture_.long_press_triggered = false;
            ESP_LOGI(TAG, "长按松开 (%d, %d)", gesture_.start_x, gesture_.start_y);
            if (gesture_callback_) {
