...
2026-04-20 复测 n=580 得 X=[22..271] Y=[0..255]，与历史一致）。
...
1. 联系屏厂 **湖南宏祺瑞**（HQR180009BH）索要 edge=0 的 V29xx 固件二进制
2. 拿到新固件后：
   - 替换 `include/axs5106l_firmware.h` 内嵌数据
   - 更新 `axs5106l_upgrade.cc` 里的目标版本号（当前对比 0x05 寄存器）
   - OTA 自动刷入：驱动已具备 `CheckAndUpgradeFirmware()` 升级通道
3. 刷完后重跑 `AXS5106L_TOUCH_DEBUG_OVERLAY=1` 验证 raw 覆盖 [0..283]×[0..239]
4. 验证通过后把 `axs5106l_touch.h` 的 `AXS5106L_TOUCH_DEBUG_OVERLAY` 改回 0

### 坐标方向核对（踩坑记录）

**重要**：chip V2905 **已内部 rotation** 输出 landscape 坐标，host 层 `TOUCH_SWAP_XY/MIRROR_X/MIRROR_Y` 必须**全 false**。不要跟 `DISPLAY_SWAP_XY/MIRROR_X` 对齐——LCD 是物理旋转，触摸是固件层旋转，两者在不同层完成。

判据：量产配置下 `[calib]` 日志应为 `raw X∈[21..273]`, `raw Y∈[0..236+]`；如果 X 范围落在 [0..239]（短边）或 Y 超过 283，说明 swap/mirror 配反了。
...
#define TOUCH_SWAP_XY      false   // chip V2905 已内部做旋转，不用 swap
